...
瀏覽器（GitHub Pages 靜態頁）
 ...
 └─ 寫：GitHub Contents API →
        ...
        intersection-reviews/ 每次勾/取消「已檢查」一個事件檔
                  ↓ push 觸發
            GitHub Action（merge-annotations.yml）
                  ↓ 自動合併 + 刪佇列
            04_osm_dataset_pipeline/outputs/lane-nav/annotations.jsonl
            04_osm_dataset_pipeline/outputs/lane-nav/intersection_reviews.jsonl（正式檔）
```
...
2. Resource owner 選 `LanePilot-Team`；Repository access：**Only select repositories** → 只勾 `LanePilot`
3. Permissions → Repository permissions → **Contents: Read and write**（其他都不用）
4. 打開標註網頁 → 上方「線上儲存設定」→ 貼上 token → 儲存設定

Token 只存在自己瀏覽器的 localStorage，不會進入網頁原始碼或 repo。
...
- 主 repo 是 private；程式端請使用帶 GitHub token 的 Contents API 讀取，或直接使用頁面上的下載功能
